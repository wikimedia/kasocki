 ...
 * @return {function}
 */
function createMatcherFunctionWithFilters(filters, safeRegexes) {
...
// This will be used to match objects using a Json Schema.
// We instantiate it as globally as possible so repeat JSON schemas
// will be bached well.
const ajv = new Ajv();

/**
 * This takes a JSON Schema and returns a new function that can be used
 * to validate (AKA match) objects against that JSON schema.
 *
 * Example:
 *  // filter for objects that have a price greater than 20
 *  var jsonSchema = { type: 'object', properties: { price: { type: 'number', maximum: 20 } } };
 *  var matcher = createMatcherFunctionJsonSchema(jsonSchema);
 *  var obj = { my: 'value', price: 19 }
 *  if (!matcher(obj)) {
 *      console.log('It didn't match!', obj, matcher.filters, matcher.errors);
 *  }
 *
 * The jsonSchema can be accessed from the returned function as matcher.filters.
 * If any validation errors are encountered, matcher will return false,
 * and the validation errors can be immediately accessed as matcher.errors.
 * The next call to matcher will overwrite matcher.errors, so if you need
 * to save them be sure to clone them to another object.
 *
 * @param  {Object}     jsonSchema
 * @return {function}
 */
function createMatcherFunctionWithJsonSchema(jsonSchema) {
    const validate = ajv.compile(jsonSchema);
    // return a new function that validates obj with jsonSchema
    let matcher = (obj) => {
        const validated = validate(obj);
        // If any error will be accessible as matcher.errors
        matcher.errors = validate.errors;
        return validated;
    };
    // Save the jsonSchema we use for matching as matcher.filters
    // (and matcher.jsonSchema)
    matcher.filters    = jsonSchema;
    matcher.jsonSchema = jsonSchema;
    matcher.validate   = validate;
    return matcher;
}
...
module.exports = {
    ...
    createMatcherFunctionWithFilters:       createMatcherFunctionWithFilters,
    createMatcherFunctionWithJsonSchema:    createMatcherFunctionWithJsonSchema
};
